...
class DictionnaireOrdonne:
    ...
    def _init_(self, base = {}, **donnees):
        
        self.cles = []    # Liste contenant les cles
        self.valeurs = []   #  Liste contenant les valeurs correspondant aucx cles

        if type(base) not in (dict, DictionnaireOrdonne):
            raise TypeError ( \
                "Le type attendu est un dictionnaire")

        # On recupère les donneds de 'base'
        for cle in base:
            self[cle] = base[cle]
